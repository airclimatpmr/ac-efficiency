...
def material_keyboard(mode: str) -> InlineKeyboardMarkup:
    """mode: "use" (списание) или "in" (приход) — определяет префикс callback_data,
    сами позиции материалов одни и те же (act_catalog.STOCK_MATERIALS)."""
    prefix = "mat" if mode == "use" else "stockin"
    rows = []
    row = []
    ...
    if row:
        rows.append(row)
    rows.append([InlineKeyboardButton("Другое", callback_data=f"{prefix}:other")])
    return InlineKeyboardMarkup(rows)
...
async def cmd_material(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_installer(chat_id):
        return
    await start_material_flow(chat_id, update.message.reply_text, mode="use")


async def start_material_flow(chat_id: int, reply_target, mode: str) -> None:
    """mode: "use" (списание) или "in" (приход). Список материалов —
    единый (act_catalog.STOCK_MATERIALS), чтобы не плодить задвоенные
    названия на складе независимо от того, каким путём материал туда попал."""
    prompt = "Какой материал списываете?" if mode == "use" else "Какой материал приходуете?"
    await reply_target(prompt, reply_markup=material_keyboard(mode))
...
async def handle_material_text(update: Update, chat_id: int, text: str) -> None:
    ...
    if state["step"] == "await_other_name":
        ...
        verb = "списать" if mode == "use" else "оприходовать"
        await update.message.reply_text(f"Сколько {verb} «{name}»? Введите число ({state['unit']}).")
        return
    ...
    if state["step"] == "await_qty":
        ...
        if auto_created:
            ...
            await notify_admins(
                None,
                f"ℹ️ Монтажник {action} материал «{name}» ({unit}), которого не было на складе — "
        ...
        source = "Приход (кнопка)" if mode == "in" else "Списать материал (кнопка)"
        await log_stock_change(name, unit, operation, qty, new_qty, chat_id, source)
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_text == BTN_STOCK_IN:
        if not is_admin(chat_id):
            return
        await start_material_flow(chat_id, update.message.reply_text, mode="in")
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("mat:") or data.startswith("stockin:"):
        mode = "use" if data.startswith("mat:") else "in"
        guard_ok = is_installer(chat_id) if mode == "use" else is_admin(chat_id)
        if not guard_ok:
            return
        code = data.split(":", 1)[1]
        verb = "списать" if mode == "use" else "оприходовать"
        if code == "other":
            PENDING_MATERIAL[chat_id] = {"mode": mode, "step": "await_other_name"}
            await reply_target("Напишите название материала:")
            return
        name, unit = MATERIAL_BY_CODE[code]
        PENDING_MATERIAL[chat_id] = {"mode": mode, "step": "await_qty", "name": name, "unit": unit}
