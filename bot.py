...
async def cmd_stock_reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обнуляет остатки ВСЕХ материалов на складе (список материалов и
    единицы измерения не трогает — только количество). Пригодится, чтобы
    стереть тестовые/пробные данные и начать реальный учёт с нуля."""
    chat_id = update.effective_chat.id
    if not is_admin(chat_id):
        return

    count = db.reset_inventory()
    await update.message.reply_text(f"Остатки обнулены у {count} материалов на складе.")
    await sync_stock_sheet()
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(chat_id):
        lines += [
            ...
            "/stock_reset — обнулить остатки всех материалов (начать учёт заново)",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 5. Для админов/монтажников этот текст точно не для КПД-калькулятора
    #    (он для обычных клиентов) — не запускаем флоу "спросить город",
    #    иначе неопознанная (например, устаревшая) кнопка может застрять,
    #    раз за разом трактуясь как ответ на вопрос про город.
    if is_installer(chat_id) or is_admin(chat_id):
        await update.message.reply_text(
            "Не понял команду. Нажмите /menu, чтобы обновить кнопки, или /help — список команд."
        )
        return

    # 6. Город ещё неизвестен вообще — это первое сообщение.
    ...
    # 7. Иначе — это модель кондиционера для расчёта КПД.
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("stock_reset", cmd_stock_reset))
