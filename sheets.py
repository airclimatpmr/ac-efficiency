...
2. Остатки И движения склада материалов — в "Склад материалов": лист
   «Остатки» (текущий снимок) и лист «Журнал» (история каждого прихода и
   расхода по датам), после каждого изменения склада (списание, приход,
   /actcalc).
...
def _get_or_create_worksheet(sh, title: str, header: list[str], rows: int = 1000, cols: int = 10):
    """Возвращает лист с нужным названием, создавая его (с заголовком),
    если такого листа в таблице ещё нет."""
    import gspread
...
STOCK_BALANCE_SHEET_NAME = "Остатки"
STOCK_JOURNAL_SHEET_NAME = "Журнал"
STOCK_JOURNAL_HEADER = [
    "Дата", "Время", "Материал", "Ед. изм.", "Операция",
    "Количество", "Остаток после", "Исполнитель", "Источник",
]


def sync_inventory(rows: list[dict]) -> None:
    """Полностью перезаписывает лист «Остатки» текущими остатками — снимок
    состояния склада на данный момент (не история движений, см.
    log_stock_movement для истории).
    ...
    ws = _get_or_create_worksheet(
        sh, STOCK_BALANCE_SHEET_NAME, ["Материал", "Ед. изм.", "Остаток", "Обновлено"]
    )
...
def log_stock_movement(
    ...
    """Добавляет одну строку в лист «Журнал» — полная история прихода и
    расхода по датам, а не только текущий остаток (тот живёт в «Остатки»).

    operation — "Приход" или "Расход". Бросает исключение при ошибке — как
    и остальные функции этого модуля, вызывающий код должен её поймать.
    """
    gc = _get_client()
    sh = gc.open_by_key(STOCK_SPREADSHEET_ID)
    ws = _get_or_create_worksheet(sh, STOCK_JOURNAL_SHEET_NAME, STOCK_JOURNAL_HEADER)
    ...
    row = [
        ...
        qty,
        balance_after,
        installer_display_name(actor_chat_id),
        source,
    ]
    ws.append_row(row, value_input_option="USER_ENTERED")
