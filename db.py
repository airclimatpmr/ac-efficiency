...
def init_db() -> None:
    ...
    _ensure_column("installations", "phone", "TEXT")
    _ensure_column("installations", "address", "TEXT")
    _ensure_column("installations", "source", "TEXT")
    _ensure_column("installations", "ocr_confidence", "TEXT")
    _ensure_column("installations", "ocr_raw_photo_file_id", "TEXT")
...
def add_installation(chat_id: int | None, client_label: str, model_text: str,
                      ...
                      install_date: date, warranty_months: int = 24,
                      phone: str | None = None, address: str | None = None,
                      source: str | None = None, ocr_confidence: str | None = None,
                      ocr_raw_photo_file_id: str | None = None) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO installations
               (chat_id, client_label, model_text, capacity_class, city,
                install_date, warranty_months, phone, address, source,
                ocr_confidence, ocr_raw_photo_file_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (chat_id, client_label, model_text, capacity_class, city,
             install_date.isoformat(), warranty_months, phone, address, source,
             ocr_confidence, ocr_raw_photo_file_id),
        )
        return cur.lastrowid
...
def list_recent_installations(limit: int = 15):
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM installations ORDER BY id DESC LIMIT ?", (limit,)
        ).fetchall()
...
def get_inventory_item(name: str):
    with get_conn() as conn:
        row = conn.execute("SELECT * FROM inventory WHERE name = ?", (name,)).fetchone()
        return dict(row) if row else None


def reset_inventory() -> int:
    """Обнуляет остатки ВСЕХ материалов на складе (список материалов и
    единицы измерения не трогает — только количество). Возвращает, сколько
    строк было обнулено. Используется, когда нужно начать учёт заново
    (например, накопились тестовые данные)."""
    with get_conn() as conn:
        cur = conn.execute("UPDATE inventory SET quantity = 0")
        return cur.rowcount
...
def list_all_acts(limit: int = 200):
    with get_conn() as conn:
        rows = conn.execute("SELECT * FROM acts ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
        return [dict(r) for r in rows]
...
def get_material_act(act_id: int):
    with get_conn() as conn:
        row = conn.execute("SELECT * FROM material_acts WHERE id = ?", (act_id,)).fetchone()
    if row is None:
        return None
    result = dict(row)
